...
public class RecordController {
    ...
    void editRecord(ActionEvent event) {
        ...
        if (selectedItem != null) {
            ...
            showWarning("Выберите запись в таблице для редактирования");
        }
    }
    ...
    void deleteRecord(ActionEvent event) {
        ...
        if (selectedItem != null) {
            ...
            alert.setContentText("Вы действительно хотите удалить запись пользователя \"" + selectedItem.getUserFio() + "\"?");
            Optional<ButtonType> result = alert.showAndWait();
            if (result.isPresent() && result.get() == ButtonType.OK) {
                new RecordService().delete(selectedItem.getRecord());
                recordObservable.remove(selectedItem);
            }
        } else {
            showWarning("Выберите запись в таблице для удаления");
        }
    }
